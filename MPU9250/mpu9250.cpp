...
void MPU9250::updateTime(Timer* t) {
    Now = t->read_us();
...
void MPU9250::initAK8963(float* destination) {
    ...
    char rawData[3];  // x/y/z gyro calibration data stored here
    ...
    writeByte(AK8963_ADDRESS, AK8963_CNTL, _Mscale << 4 | Mmode); // Set magnetometer data resolution and sample ODR
    wait_ms(10);
}
...
void MPU9250::initMPU9250() {
    ...
    delt_t = 0;
    count = 0; sumCount = 0;
    deltat = 0.0f; sum = 0.0f;
    lastUpdate = 0; firstUpdate = 0;
    Now = 0;

    for (int countium = 0; countium < 3; countium++)
    {
        magCalibration[countium] = 0;
        magbias[countium] = 0;
        gyroBias[countium] = 0;
        accelBias[countium] = 0;
...
void MPU9250::readBytes(uint8_t address, uint8_t subAddress, uint8_t count, char* dest) {
    char* cmd;
    *cmd = subAddress;
    i2c.write(address, cmd, 1);

    i2c.read(address, dest, count);
}